...
頁面每 15 秒輪詢一個幾十位元組的 `version.json`；版本一變才去重取圖片。**圖片網址
是固定的**，前面每一層快取照常命中——版本只用來判斷「有沒有新的」，不進網址。

開著的頁面本來就不會重新請求圖片：`<img>` 載入一次就固定在那裡，跟快取標頭無關。
這個輪詢就是補上那個「該再問一次」的理由，而且只在真的有新圖時才問。載入時也會輪詢
一次，所以即使 HTML 來自快取也會立刻更正。頁首倒數顯示距離下次重畫還有多久。
