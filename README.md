...
defaults:
  ...
  area: first
  tz: Asia/Taipei
  # 省略 ranges 就是 MRTG 的四層：1d / 1w / 1m / 1y，各自以自己的 step 重畫
...
頁面用同樣的數字算出同樣的邊界，所以兩邊不必互相溝通——**沒有版本檔、沒有輪詢**。
那些數字就是繪製週期，以及每一張圖各自的重畫頻率。

每輪會**提前 5 秒開始**畫，邊界到達時檔案就已經就位。某一輪如果畫得比提前量還久，
log 會警告。

頁首倒數到下一個邊界，時間到就換掉那一刻到期的圖。**圖片網址是固定的**（沒有版本參數），所以前面
每一層快取照常命中。
...
**每一層尺度以自己的 step 為週期重畫**，因為圖上一欄就是一個 step 寬。年圖要到明天
才會多出一欄，每 5 分鐘重畫一次等於一天花掉 288 次查詢，換回硬碟上那張一模一樣的圖；
每 15 秒一次就是 5,760 次。
...
defaults:
  ranges:
    - {name: 1d, from: -1d,   step: 5m}             # 每一輪都畫
    - {name: 1y, from: -365d, step: 1d, every: 6h}  # 或者自己指定

graphs:
  - name: requests
    every: {1w: 5m, 1m: 5m, 1y: 5m}   # 被開著一直看的那一張
```

range 上的 `every` 定下整層階梯；graph 上的 `every` 以尺度名稱為鍵，只覆寫其中一層，
所以需要看得勤的那張圖不必把整組 ranges 重寫一遍。鍵若不是那張圖有的尺度，載入時就
報錯，而不是靜靜地什麼都不做。

每個頻率都會向上取整成 `output.interval` 的整數倍，因為只有繪製那一輪會動到檔案。
這正是頁面倒數的邊界與繪製落下的邊界完全重合的原因：**每個 `<img>` 知道自己的週期**，
只在自己該換的時候重抓自己，而頁首倒數的是這一頁上最快到期的那一張。

以 MRTG 的四層尺度來說，每張圖每輪平均 1.2 次查詢，而不是 4 次。查詢失敗的那一層不
算畫過，所以下一輪會再試，而不是讓年圖一路停在舊圖直到明天。
...
這道階梯在長尺度那端會用完預算。一年份的 10 秒樣本是每條 series 三百萬點，所以年圖
只能取到月圖的 step，5 秒的突波進到那條峰值線已經小了幾百倍。`peak_expr` 讓那條線改
從別的地方畫——一條在資料源自己的解析度上先把最大值取好的 recording rule：

```yaml
series:
  - expr: sum(rate(nginx_http_bytes_total[$step])) * 8 / 1e6
    peak_expr: max_over_time(nginx:bytes:peak30s[$step]) * 8 / 1e6
    peak_ranges: [1w, 1m, 1y]   # 省略就是每一層
```

`peak_expr` 是整條寫完的，`max_over_time` 也在裡面，所以這裡的 `$step` 是外層的桶，
不是桶裡樣本的間隔：rule 存的已經是最大值，再套一層子查詢只會跨過它存在的意義——用
40 分鐘去取樣一條 5 分鐘的 rule，是八個值留一個丟七個。另一條路是把上限抬高。`source.max_subquery_points`（預設 100000，對上
`-search.maxPointsSubqueryPerTimeseries`）就是這道階梯的預算來源；子查詢不管走過多少
點，回傳的都是每個桶一個點——一年每 30 秒取一次峰值，資料源要走一百萬點，render 只收
一千多點。這邊和資料源兩邊一起調高，長尺度就直接讀本來就存在的原始樣本，不必養一條
rule，也不必等歷史長出來。

`peak_ranges` 把最細那端留在
階梯上，那裡本來就已經讀在資料源的解析度，換過去沒有好處。改走這條路的尺度不再受子
查詢點數上限約束，因為它不再發子查詢。
