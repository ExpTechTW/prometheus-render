...
單純重建 `<img>` 元素是不夠的：瀏覽器以網址為鍵決定要不要重用，指向同一個位址的新
元素會直接從自己的快取取用，根本不碰網路——HTML 規範甚至會把連續設定 `src` 併成
一次載入以避免請求。所以先用 `fetch(url, {cache: "reload"})` 把瀏覽器自己那份換掉，
再重建元素；元素這時就地拿到新的位元組，不會再發第二次請求。
